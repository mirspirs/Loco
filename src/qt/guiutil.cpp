...
{
    // Convert loco:// to loco:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("loco://"))
    {
        uri.replace(0, 12, "loco:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Loco.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ILocoLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ILocoLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "loco.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Loco-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  loco-qt [" + tr("command-line options") + "]                     " + "\n";
