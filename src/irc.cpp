...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #lococoinTEST\r");
            Send(hSocket, "WHO #lococoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #lococoin\n");
            // randomly join #lococoin00-#lococoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #lococoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #lococoin%02d\r", channel_number).c_str());
        };
